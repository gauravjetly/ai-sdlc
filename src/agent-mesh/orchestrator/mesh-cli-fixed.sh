...
ensure_dirs() {
  ...
  for _agent in "${agents[@]}"; do
    mkdir -p "${MESH_BASE}/bus/inboxes/${_agent}"
    mkdir -p "${MESH_BASE}/bus/outboxes/${_agent}"
  done
}
...
EOF
  ...
  if [ "$to" = "all" ]; then
    ...
    local agents=("conductor" "ba" "jets" "ux" "engineer" "security" "qa" "atlas" "customer" "ask-tom" "tracker" "finops")
    local _bcast_agent
    for _bcast_agent in "${agents[@]}"; do
      if [ "$_bcast_agent" != "$from" ]; then
        local broadcast_id
...
msg['id'] = '${broadcast_id}'
msg['receiver'] = '${_bcast_agent}'
print(json.dumps(msg, indent=2))
")
        echo "$broadcast_msg" > "${MESH_BASE}/bus/inboxes/${_bcast_agent}/${priority}-${ts_safe}-${broadcast_id}.json"
      fi
    done
